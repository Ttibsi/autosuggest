...
void collate_words(
    Trie* root, char* prefix, Node* viable_words, Arena* dll_arena, int extant_lines) {
...
void generate_viable_words(
    Node* viable_words, Trie* root, char* input, Arena* dll_arena, int selected_len,
    Node* selected_words) {
    clear_screen();
    ...
    while (true) {
        ...
        if (c == CTRL_KEY('c')) {
            ...
            if (input_len > 0) {
                ...
                if (input_len >= 3) {
                    generate_viable_words(
                        &viable_words, root, input, &dll_arena, nodeLen(&selected_words),
                        &selected_words);
                } else {
                    clear_line();
                }
